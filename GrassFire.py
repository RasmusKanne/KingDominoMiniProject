...
def ignite_pixel(image, coordinate, id):
    ...
    while len(burn_queue) > 0:
        ...
        if image[y, x] == current_category:
            image[y, x] = id

            if x + 1 < image.shape[1] and image[y, x + 1] == current_category:
                burn_queue.append((y, x + 1))
            if y + 1 < image.shape[0] and image[y + 1, x] == current_category:
                burn_queue.append((y + 1, x))
            if x - 1 >= 0 and image[y, x - 1] == current_category:
                burn_queue.append((y, x - 1))
            if y - 1 >= 0 and image[y - 1, x] == current_category:
                burn_queue.append((y - 1, x))
